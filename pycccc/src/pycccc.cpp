...
#define LayerFactoryDeclear(n)                  \
    static cccc::Layer* LayerCreator##n(void*); \
    static void LayerDestroyer##n(cccc::Layer*, void*);
...
#define LayerFactoryDefine(n)                                  \
    static cccc::Layer* LayerCreator##n(void* p)               \
    {                                                          \
    ...
    static void LayerDestroyer##n(cccc::Layer* layer, void* p) \
    {                                                          \
...
PYBIND11_MODULE(cccc, m)
{
    ...
    .def(py::init([](py::buffer const b) {
        ...
        {
            std::stringstream ss;
            ss << "convert numpy.ndarray to cccc.Mat only dims <=4 support now, but given " << info.ndim;
            pybind11::pybind11_fail(ss.str());
        }
        ...
        else if (info.ndim == 3)
        {
            ...
            // in cccc, buffer to construct cccc::Mat need align to cccc::alignSize
    ...
    "row", [](Mat& m, int y) {
        ...
        {
            std::stringstream ss;
            ss << "get cccc.Mat row only elempack 1 support now, but given " << m.elempack;
            pybind11::pybind11_fail(ss.str());
        }
        ...
        default:
            std::stringstream ss;
            ss << "cccc.Mat row elemsize " << m.elemsize << "not support now";
            pybind11::pybind11_fail(ss.str());
        }
        return py::memoryview::from_buffer(m.row<float>(y), {m.w}, {sizeof(float)});
    },
    ...
    .def("__repr__", [](const Mat& m) {
        std::stringstream ss;
        ss << "<cccc.Mat w=" << m.w << " h=" << m.h << " d=" << m.d << " c=" << m.c << " dims=" << m.dims
    ...
    py::enum_<cccc::Mat::PixelType>(mat, "PixelType")
    .value("PIXEL_CONVERT_SHIFT", cccc::Mat::PixelType::PIXEL_CONVERT_SHIFT)
    .value("PIXEL_FORMAT_MASK", cccc::Mat::PixelType::PIXEL_FORMAT_MASK)
    .value("PIXEL_CONVERT_MASK", cccc::Mat::PixelType::PIXEL_CONVERT_MASK)

    .value("PIXEL_RGB", cccc::Mat::PixelType::PIXEL_RGB)
    .value("PIXEL_BGR", cccc::Mat::PixelType::PIXEL_BGR)
    .value("PIXEL_GRAY", cccc::Mat::PixelType::PIXEL_GRAY)
    .value("PIXEL_RGBA", cccc::Mat::PixelType::PIXEL_RGBA)
    .value("PIXEL_BGRA", cccc::Mat::PixelType::PIXEL_BGRA)

    .value("PIXEL_RGB2BGR", cccc::Mat::PixelType::PIXEL_RGB2BGR)
    .value("PIXEL_RGB2GRAY", cccc::Mat::PixelType::PIXEL_RGB2GRAY)
    .value("PIXEL_RGB2RGBA", cccc::Mat::PixelType::PIXEL_RGB2RGBA)
    .value("PIXEL_RGB2BGRA", cccc::Mat::PixelType::PIXEL_RGB2BGRA)

    .value("PIXEL_BGR2RGB", cccc::Mat::PixelType::PIXEL_BGR2RGB)
    .value("PIXEL_BGR2GRAY", cccc::Mat::PixelType::PIXEL_BGR2GRAY)
    .value("PIXEL_BGR2RGBA", cccc::Mat::PixelType::PIXEL_BGR2RGBA)
    .value("PIXEL_BGR2BGRA", cccc::Mat::PixelType::PIXEL_BGR2BGRA)

    .value("PIXEL_GRAY2RGB", cccc::Mat::PixelType::PIXEL_GRAY2RGB)
    .value("PIXEL_GRAY2BGR", cccc::Mat::PixelType::PIXEL_GRAY2BGR)
    .value("PIXEL_GRAY2RGBA", cccc::Mat::PixelType::PIXEL_GRAY2RGBA)
    .value("PIXEL_GRAY2BGRA", cccc::Mat::PixelType::PIXEL_GRAY2BGRA)

    .value("PIXEL_RGBA2RGB", cccc::Mat::PixelType::PIXEL_RGBA2RGB)
    .value("PIXEL_RGBA2BGR", cccc::Mat::PixelType::PIXEL_RGBA2BGR)
    .value("PIXEL_RGBA2GRAY", cccc::Mat::PixelType::PIXEL_RGBA2GRAY)
    .value("PIXEL_RGBA2BGRA", cccc::Mat::PixelType::PIXEL_RGBA2BGRA)

    .value("PIXEL_BGRA2RGB", cccc::Mat::PixelType::PIXEL_BGRA2RGB)
    .value("PIXEL_BGRA2BGR", cccc::Mat::PixelType::PIXEL_BGRA2BGR)
    .value("PIXEL_BGRA2GRAY", cccc::Mat::PixelType::PIXEL_BGRA2GRAY)
    .value("PIXEL_BGRA2RGBA", cccc::Mat::PixelType::PIXEL_BGRA2RGBA);
...
#if cccc_STRING
    .def(
    "register_custom_layer", [](Net& net, const char* type, const std::function<cccc::Layer*()>& creator, const std::function<void(cccc::Layer*)>& destroyer) {
...
#endif //cccc_STRING
    .def(
    "register_custom_layer", [](Net& net, int index, const std::function<cccc::Layer*()>& creator, const std::function<void(cccc::Layer*)>& destroyer) {
        if (g_layer_factroy_index == g_layer_factroys.size())
        {
            std::stringstream ss;
            ss << "python version only support " << g_layer_factroys.size() << " custom layers now";
    ...
    py::enum_<cccc::BorderType>(m, "BorderType")
    .value("BORDER_CONSTANT", cccc::BorderType::BORDER_CONSTANT)
    .value("BORDER_REPLICATE", cccc::BorderType::BORDER_REPLICATE);
